...
(defn- submit-action! [mom-url action]
  (println "Submitting action to MoM:" action)
  (try
    ...
    (catch Exception e
      (println "Failed to submit action:" (.getMessage e)))))
...
(defn handle-event [request]
  (let [event (:body request)
    ...
    (if (= (str event-type) "system/hello")
      (do
        (println ">>> HelloWorld Agent says: Hello, Intergraph! <<<")
...
(defn -main [& args]
  (let [port (or (first args) "9090")]
    (println "Starting Distributed Agent on port" port)
